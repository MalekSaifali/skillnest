...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (localStorage.getItem('sn_token')) router.push('/dashboard');
  }, [router]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await axios.post('http://15.206.124.18:4000/api/auth/login', form);
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .login-input { width: 100%; padding: 14px 16px; background: #1e293b; border: 1.5px solid #334155; border-radius: 12px; color: white; font-size: 15px; outline: none; box-sizing: border-box; transition: border-color 0.2s, box-shadow 0.2s; }
      ...
      <div style={{ flex: 1, background: 'linear-gradient(135deg, #1e1b4b 0%, #312e81 50%, #1e1b4b 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 60, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: '20%', right: '15%', width: 160, height: 160, borderRadius: '50%', background: 'rgba(99,102,241,0.2)', filter: 'blur(30px)', animation: 'pulse 5s ease-in-out infinite 1s' }} />
        ...
        <div style={{ position: 'absolute', top: '55%', right: '8%', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, padding: '10px 16px', fontSize: 13, color: '#86efac', animation: 'float 6s ease-in-out infinite 2s', backdropFilter: 'blur(8px)' }}>🤖 AI/ML Engineer</div>
        <div style={{ position: 'relative', textAlign: 'center', animation: 'fadeUp 0.6s ease' }}>
          ...
          <div style={{ display: 'flex', gap: 32, justifyContent: 'center' }}>
            {[{ num: '500+', label: 'Professionals' }, { num: '50+', label: 'Skills' }, { num: '100%', label: 'Free' }].map((stat, i) => (
              <div key={i} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 26, fontWeight: 800, color: '#c4b5fd' }}>{stat.num}</div>
                <div style={{ fontSize: 12, color: '#6d7fc4', marginTop: 2 }}>{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ width: 480, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px', background: '#0f172a', borderLeft: '1px solid #1e293b' }}>
        <div style={{ width: '100%', animation: 'fadeUp 0.5s ease' }}>
          ...
          {error && (
            <div style={{ background: '#3f1a1a', border: '1px solid #7f1d1d', borderRadius: 10, padding: '12px 16px', marginBottom: 20, color: '#fca5a5', fontSize: 14 }}>
              ⚠️ {error}
            </div>
          )}
